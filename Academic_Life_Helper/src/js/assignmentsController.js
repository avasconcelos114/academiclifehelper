    ...
    function AssignmentsController(
        ...
        $scope.getAssignmentList = function(){
            ...
            function(success){
                ...
                for(let i = 0; i <= success.data.length - 1; i++) {
                    let start_date_object = { _id: success.data[i]._id, date : new Date(success.data[i].startDate) }
                    let due_date_object   = { _id: success.data[i]._id, date : new Date(success.data[i].dueDate) }
        ...
        $scope.findDateById = function(type, id) {
            if (type === 'dueDate' ){
                ...
                let index = $scope.assignment_start_dates.findIndex(x => x._id === id)
                return index;
            }
        };
        ...
        $scope.findIndexById = function(type, id) {
            if(type === 'activity') {
                let index = $scope.activities.findIndex(x => x._id === id)
                return index;
            } else if (type === 'assignment') {
                let index = $scope.assignments.findIndex(x => x._id === id)
                return index;
            }
        };

        $scope.checkDateStatus = function(assignment) {
            let currentDateUnix = new Date().getTime() / 1000;
            let thisDateUnix = new Date(assignment.dueDate).getTime() / 1000;
            let currentDate =  $filter("date")(new Date(), "yyyyMMdd");
            let thisDate =  $filter("date")(assignment.dueDate, "yyyyMMdd");

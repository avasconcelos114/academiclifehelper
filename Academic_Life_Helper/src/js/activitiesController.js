(function() {
    ...
    ActivitiesController.$inject = ['$scope', '$http', '$cookies', '$location', '$mdDialog'];

    function ActivitiesController($scope, $http, $cookies, $location, $mdDialog) {
        // initialized variables to be shown on screen
        $scope.activities = [];
  
        // selected array values
        if($cookies.get('selected_activity_id') >= 0) {
            $scope.selected_activity_id = parseInt($cookies.get('selected_activity_id'));
        } else {
        ...
        if($scope.selected_activity_id >= 0) {
            $scope.selectActivity($scope.selected_activity_id);
        }
